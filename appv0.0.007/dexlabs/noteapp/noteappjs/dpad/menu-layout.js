(function () {
  ...
  const MENU_TREE = [
    ...
    { sep: true },
    {
      ...
      children: [
        ...
        { id: 'dexTbDiffSave',       label: 'Save Text',        icon: ICONS.save },
        { sep: true },
  ...
  function updateDiffEntryVisibility() {
    ...
    if (!trigger) return;
    const route = ctx.parseNoteRoute ? ctx.parseNoteRoute(window.location.pathname) : null;
    trigger.style.display = (route && route.mode === 'diffusion') ? '' : 'none';
  }
