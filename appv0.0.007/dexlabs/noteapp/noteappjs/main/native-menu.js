(function () {
  ...
  function hookTextarea(id) {
    // Tracks the currently-bound element (not just a one-shot flag) so a
    // recreated #mermaid-code textarea (e.g. leaving/re-entering Mermaid
    // mode) gets rehooked instead of silently losing the native menu.
    let boundEl = null;
    const onSelChange = function () {
      const ta = this;
    ...
    (function tryHook() {
      ...
      if (ta && ta !== boundEl) {
        boundEl = ta;
        ['select', 'pointerup', 'keyup'].forEach((evt) => ta.addEventListener(evt, onSelChange));
      }
