...
function closeSidebar() {
  ...
  function buildSubLevel(container, dataContainer, depth) {
    ...
    Array.from(dataContainer.children).forEach(dataDiv => {
      ...
      if (isCollapse) {
        ...
        const batchAttr = dataDiv.getAttribute('data-batch');
        if (batchAttr) item.dataset.batch = batchAttr;
  ...
  function renderPinnedSection() {
    ...
    section.querySelectorAll('.sidebar2-pinned-item').forEach((btn, idx) => {
      const pinBtn = btn.querySelector('.sidebar2-pin-btn');
      const entry = pins[idx];
      pinBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        togglePin(entry);
      });
