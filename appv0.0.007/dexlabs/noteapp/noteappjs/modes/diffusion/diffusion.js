...
function diffusionIfEnabled() {
  if (window.dexMode && !window.dexMode.enabled) return;
  diffusion();
}
[diffElements.raw, diffElements.morph].forEach(t => t.addEventListener('input', diffusionIfEnabled));
if (diffElements.inputType) diffElements.inputType.addEventListener('change', diffusionIfEnabled);
if (diffElements.trimSpaces) diffElements.trimSpaces.addEventListener('change', diffusionIfEnabled);
diffusion();
