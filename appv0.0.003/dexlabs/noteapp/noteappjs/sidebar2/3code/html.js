...
export async function showTextToTableOptionsModal() {
...
export function handleRemoveHtml() {
  if (!currentNote || !noteTextarea) return;
  try {
    noteTextarea.value = htmlToPlainText(noteTextarea.value);
    if (typeof updateNoteMetadata === "function") updateNoteMetadata();
    showNotification("HTML removed");
  } catch (err) {
    console.error("RemoveHTML error:", err);
    showNotification("Remove HTML failed: " + (err && err.message ? err.message : String(err)));
  }
}

export function handleEscapeHtml() {
  if (!currentNote || !noteTextarea) return;
  noteTextarea.value = escapeHtmlText(noteTextarea.value);
  if (typeof updateNoteMetadata === "function") updateNoteMetadata();
  showNotification("HTML escaped");
}

export function handleUnescapeHtml() {
  if (!currentNote || !noteTextarea) return;
  noteTextarea.value = unescapeHtmlText(noteTextarea.value);
  if (typeof updateNoteMetadata === "function") updateNoteMetadata();
  showNotification("HTML unescaped");
}

export async function handleTextToTable() {
  if (!currentNote || !noteTextarea) return;
  const params = await showTextToTableOptionsModal();
  if (!params) return;
  try {
    noteTextarea.value = textToHtmlTable(noteTextarea.value, params);
    if (typeof updateNoteMetadata === "function") updateNoteMetadata();
    showNotification("Text converted to table");
  } catch (err) {
