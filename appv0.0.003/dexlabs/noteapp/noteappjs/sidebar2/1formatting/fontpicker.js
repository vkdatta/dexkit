...
export function listFontChoices() {
  return Object.keys(FONTS).map((key) => ({ value: key, label: FONTS[key].label }));
}

export function applyDesignerFont(text, textKey, numKey) {
  return convert(toAscii(text), textKey, numKey);
}
