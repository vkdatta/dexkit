(function () {
  ...
  style.textContent = `
    ...
    /* ====== ULTIMATE D-PAD — Compass + Center Dragger ======
       Layout: single 200px circle. 8 direction buttons positioned absolutely
       at compass points (N, NE, E, SE, S, SW, W, NW) around a center dragger.
       Outer ring has an animated RGB rainbow glow (gaming controller LED). */
    @keyframes dexRgbSpin {
      0%   { --rgb-angle: 0deg; }
    ...
    #dexCursorControls {
      ...
      /* RGB rim: uses conic-gradient bordered ring via padding trick */
      padding: 4px;
      background:
        conic-gradient(from var(--rgb-angle, 0deg),
          #FF00AA, #B026FF, #00A6FF, #00E5FF,
          #00FF88, #FFD700, #FF6B00, #FF00AA);
      animation: dexRgbSpin 8s linear infinite;
      transition: box-shadow 0.2s ease, transform 0.15s ease;
    }
    /* Inner black disc — sits inside the conic-gradient rim */
    #dexCursorControls::before {
      content: '';
      position: absolute;
      inset: 4px;
      border-radius: 50%;
      background: radial-gradient(circle at 50% 45%, #1A1F24 0%, #0A0D10 75%);
      z-index: 0;
      box-shadow: inset 0 2px 6px rgba(0,0,0,0.6),
                  inset 0 -2px 8px rgba(0,212,170,0.05);
    }
    #dexCursorControls:hover {
      box-shadow: 0 0 32px rgba(176, 38, 255, 0.35),
    ...
    /* Compass positions — 8 points around center. Center of 200px widget
       is at (100,100). Buttons are 34px so offset is -17px. Radius = 75px. */
    #dexCursorControls .cmp-n  { left: 83px; top: 8px;   } /* N   */
    #dexCursorControls .cmp-ne { left: 141px; top: 25px; } /* NE  */
    #dexCursorControls .cmp-e  { left: 158px; top: 83px; } /* E   */
    #dexCursorControls .cmp-se { left: 141px; top: 141px;} /* SE  */
    #dexCursorControls .cmp-s  { left: 83px; top: 158px; } /* S   */
    #dexCursorControls .cmp-sw { left: 25px; top: 141px; } /* SW  */
    #dexCursorControls .cmp-w  { left: 8px;  top: 83px;  } /* W   */
    #dexCursorControls .cmp-nw { left: 25px; top: 25px;  } /* NW  */

    /* ====== CENTER DRAGGER — inside the D-Pad ====== */
    #dexCursorControls .dex-center-drag {
      position: absolute;
      left: 74px;  /* (200 - 52) / 2 = 74 */
      top:  74px;
      width: 52px;
      height: 52px;
      border-radius: 50%;
      background: radial-gradient(circle at 35% 30%, #2A3239 0%, #12171B 100%);
    ...
    #dexCursorControls .dex-center-drag .material-symbols-rounded {
      font-size: 24px;
      transition: transform 0.2s ease;
    }
    #dexCursorControls .dex-center-drag.dragging .material-symbols-rounded {
      transform: scale(1.15);
    }

    /* Direction indicator ring on center dragger */
  ...
  cursorControls.innerHTML =
    ...
    // Center dragger — lives inside the D-Pad
    '<div class="dex-center-drag" id="dexCenterDrag" aria-label="Drag to select">' +
      '<span class="material-symbols-rounded">' + ICONS.drag + '</span>' +
    '</div>';
  ...
  function clampCursor(left, top) {
    const cw = cursorControls.offsetWidth || 200;
