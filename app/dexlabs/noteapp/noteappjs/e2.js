(function () {
  ...
  style.textContent = `
    ...
    #dexCursorControls.selecting {
      border-color: rgba(100, 180, 255, 0.45);
      box-shadow: 0 8px 24px rgba(40, 120, 220, 0.28);
    }
    ...
    #dexCursorControls .dex-cursor-btn.select-mode {
      background: rgba(100, 180, 255, 0.18);
      border-color: rgba(100, 180, 255, 0.35);
      color: #8ec5ff;
    }

    #dexCursorControls .dex-cursor-btn.select-mode:active {
  ...
  menu.innerHTML =
    ...
    '<button type="button" class="dex-tb-item" id="dexTbCursor">' +
      '<span class="material-symbols-rounded">' +
  ...
  let cursorSelectMode = false;
  let cursorAnchor = null;

  function cursorControlsOpen() {
    return cursorControls.classList.contains(
      'open'
    );
  }
  ...
  function clampCursor(
    ...
    const cw =
      cursorControls.offsetWidth ||
      130;

    const ch =
      cursorControls.offsetHeight ||
      200;
  ...
  function applyCursorPos(pos) {
    cursorControls.style.left =
      pos.left + 'px';

    cursorControls.style.top =
      pos.top + 'px';
  }
  ...
  function openCursorControls() {
    ...
    if (cm) {
      ...
      cursorControls.classList.remove(
        'selecting'
      );

      // Do not focus CodeMirror here:
      // focusing its hidden textarea opens
  ...
  cursorControls.addEventListener(
    'pointerdown',
    (e) => {
      if (
        e.target.closest(
      ...
      cursorDrag = {
        ...
        startLeft:
          cursorControls.offsetLeft,

        startTop:
          cursorControls.offsetTop,

        moved: false
      };

      try {
        cursorControls.setPointerCapture(
          e.pointerId
        );
      } catch (_e) {}
    }
  ...
  function endCursorDrag(e) {
    ...
    try {
      cursorControls.releasePointerCapture(
        cursorDrag.pointerId
      );
    } catch (_e) {}
  ...
  // No cm.focus() here:
  // movement should not summon
  // the mobile keyboard.

  curUp.addEventListener(
    'click',
    () =>
      moveCursor(
        'up',
  ...
  // Long-press the cursor panel
  // (outside a movement button) to
  // toggle selection mode.
  //
  // This avoids hijacking ordinary
  // movement taps.

  let selectionToggleTimer =
    null;
      ...
      selectionToggleTimer =
        setTimeout(
          () => {
            ...
            cursorControls.classList.toggle(
              'selecting',
              cursorSelectMode
            );
          },
    ...
    cmEl.addEventListener(
      ...
      (e) => {
        ...
        try {
          ...
          dragSelectState.lastPos =
            dragSelectState.startPos;
        } catch (_e) {}

        try {
          cmEl.setPointerCapture(
            e.pointerId
          );
        } catch (_e) {}
      }
    ...
    cmEl.addEventListener(
      ...
      (e) => {
        ...
        if (
          ...
          clearTimeout(
            dragSelectState.longPressTimer
          );
