// ============================================================================
// DexLabs — Find & Replace, CodeMirror-aware.
//
// Same UI (find-replace-menu with inputs, buttons, keyboard shortcuts) as the
// original. The visual match highlighting was previously rendered by writing
// HTML into #findBackdrop that overlaid the textarea. In the CodeMirror world
// the editor covers the backdrop, so we render matches via cm.markText() and
// scroll-into-view via cm.scrollIntoView() instead.
//
// Search, navigation (prev/next), and replace (single / range / all) still
// read and write noteTextarea.value — that routes through editor-adapter's
// facade into CM, so the note is edited normally and undo/redo works.
// ============================================================================

// Small helper — kept exported so any external caller that used it still can
// (no-op now, since we don't populate #findBackdrop).
export function openfindbackdrop() { /* legacy no-op */ }
...
export function createFindAndReplace() {
  ...
  // CodeMirror marks currently in the doc — we clear these before each
  // renderMatches() run.
  const cmMarks = [];
  ...
  // ---------------------------------------------------------------------------
  // Match highlighting via CodeMirror's markText. Each mark spans one match
  // and carries either the "hl-match" or "hl-current" CSS class so the styles
  // from find.css apply exactly as before.
  // ---------------------------------------------------------------------------
  ...
  function focusCurrentMatch() {
    ...
    // Place the cursor at the match end so subsequent typing lands where the
    // user was looking. Then scroll it into view with a comfortable margin.
    try { d.setSelection(fromPos, toPos); } catch (e) {}
    try { c.scrollIntoView({ from: fromPos, to: toPos }, 80); } catch (e) {}
  }

  function updateCaretColor() { /* CM manages caret rendering itself */ }
  ...
  // Batched replace using CM's document operations — each replaceRange is
  // one undoable action, but wrapping in cm.operation() coalesces them into
  // a single visible history entry.
  function executeReplace() {
    ...
    // Sort descending so earlier positions aren't invalidated by later edits.
  ...
  // Toggle function returned to the caller — bound to window.findandreplace.
  return function () {
    const els = getElements();
    ...
    if (isOpen) {
      ...
      // Prefill the find input from the current selection (if any).
      const c = cm();
      if (c && els.findInput) {
      ...
      state.matches = [];
      state.currentIndex = -1;
      // Clear the (unused) legacy backdrop just in case some external code left content there.
