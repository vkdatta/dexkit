(function () {
  ...
  */
  ...
  // Mobile: block native touch gestures on the center handle itself so the
  // browser doesn't hijack the pointer stream mid-drag.
  centerHandle.addEventListener('touchstart', (e) => {
    if (e.touches.length === 1) e.preventDefault();
  }, { passive: false });
  centerHandle.addEventListener('touchmove', (e) => {
    if (centerDrag) e.preventDefault();
  }, { passive: false });
  ...
  function attachMobileDragSelect() {
    ...
    scroller.addEventListener('touchmove', (e) => {
      // Prevent native gestures during our drag-select OR while center
      // handle is being dragged (the finger will be over the editor area).
      if ((dragSelectState && dragSelectState.isDragging) || centerDrag) {
        e.preventDefault();
      }
    }, { passive: false });
    ...
    scroller.addEventListener('pointerdown', (e) => {
      ...
      // Ignore events that originated on our own overlay widgets — they
      // manage their own drags and pointer capture.
      if (e.target && e.target.closest &&
          (e.target.closest('#dexCenterHandle') ||
           e.target.closest('#dexCursorControls') ||
           e.target.closest('#dexToolbarBtn') ||
           e.target.closest('#dexToolbarMenu'))) return;
