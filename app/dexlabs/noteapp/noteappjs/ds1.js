(function () {
  ...
  // ----------------------------------------
  //  Hide toolbar on homepage (no editor)
  // ----------------------------------------
  ...
  function isEditorPage() {
    // Detect if a CodeMirror instance exists – this indicates a note page.
    // The homepage should not have one.
    return !!(window.dexEditor && window.dexEditor.cm);
  }

  // If no editor after a short delay, hide toolbar permanently.
  ...
  // ----------------------------------------
  //  Positioning & drag logic (unchanged)
  // ----------------------------------------
  function loadPos() {
    try {
  ...
  // ----------------------------------------
  //  Auto-open on selection / close on cursor move
  // ----------------------------------------
  function attachEditorListeners() {
    ...
    const handler = function() {
      if (!menuOpen()) {
        // If menu is closed, check if selection exists → open it
        const sel = cm.getSelection();
        if (sel && sel.length > 0) {
          openMenu();
        }
      ...
      // Menu is open: react to cursor/selection changes
      const sel = cm.getSelection();
      if (sel && sel.length > 0) {
        // Selection still exists → reposition menu
        positionMenu();
      } else {
  ...
  //  Initialization
  // ----------------------------------------
  // Check if we are on an editor page; if not, hide toolbar permanently.
  checkEditorPresence();

  // Also re-check when the page might dynamically load an editor.
  // Use MutationObserver or simply re-run after a delay.
  // We already have a retry in checkEditorPresence.
})();