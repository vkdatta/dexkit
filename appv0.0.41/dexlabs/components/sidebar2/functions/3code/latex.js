...
function readBraceGroup(str, pos) {          
    ...
    if (c === '\\') {
      ...
      if (m) {
        ...
        if (str[j] === '{') {
          ...
          push();
          parseProse(inner, mergeCtx(ctx, FMT[name] || {}), out);        
          i = end; continue;
        }
        i = j; continue;                                                 
      }
...
function buildBodyAuto(input, stats) {
  ...
  for (const line of input.split('\n')) {
    ...
    if (isMathLine(line)) {
      ...
      paras.push(`<w:p>${proseToXml(line)}</w:p>`);        
    }
  }
  if (!paras.length) paras.push('<w:p></w:p>');
  return paras.join('');
